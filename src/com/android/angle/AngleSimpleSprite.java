...
public class AngleSimpleSprite extends AngleAbstractSprite
{
	...
	{
		init(width,height,resourceId,cropLeft,cropTop,cropWidth,cropHeight,1,1);
	}
	 ...
	 * @param frameColumns Number of frames horizontally in texture
	 */
	public AngleSimpleSprite(int width, int height, int resourceId, int cropLeft,
			int cropTop, int cropWidth, int cropHeight, int frameCount, int frameColumns)
	{
		init(width,height,resourceId,cropLeft,cropTop,cropWidth,cropHeight,frameCount,frameColumns);
	}
	...
	protected void setFrame(int frame)
	{
		if (frame<mFrameCount)
		{
			...
			mTextureIV[1] = mCropBottom+((mFrame/mFrameColumns)*-mTextureIV[3]);// Vcr
		}
	}
