...
{
	private static final String LOG_TAG = "XMLMapTransformer";
	private AngleTileMap map;
	...
	private void reflectInvokeMethod(Object invokeVictim, Method method, String[] args) throws Exception
	{
		...
		for (int i = 0; i < parameterTypes.length; i++)
		{
			...
			{
				Log.d(LOG_TAG,"Unsupported argument type " + parameterTypes[i].getName() + ", defaulting to java.lang.String");
				conformingArguments[i] = args[i];
			}
		}
	...
	private void setOrientation(String o)
	{
		...
		{
			map.setOrientation(AngleTileMap.MDO_ISO);
		}
		else if ("orthogonal".equalsIgnoreCase(o))
		{
			map.setOrientation(AngleTileMap.MDO_ORTHO);
		}
		else if ("hexagonal".equalsIgnoreCase(o))
		{
			map.setOrientation(AngleTileMap.MDO_HEX);
		}
		else if ("shifted".equalsIgnoreCase(o))
		{
			map.setOrientation(AngleTileMap.MDO_SHIFTED);
		}
		else
		{
			Log.w(LOG_TAG,"Unknown orientation '" + o + "'");
		}
	}
		...
		if (nnm != null)
		{
			for (int i = 0; i < nnm.getLength(); i++)
			{
				...
				try
				{
					...
					{
						Log.w(LOG_TAG,"Unsupported attribute '" + n.getNodeName() + "' on <" + node.getNodeName() + "> tag");
					}
				}
				catch (Exception e)
				{
		...
		else
		{
			...
			for (int i = 0; i < nl.getLength(); i++)
			{
				...
				if ("data".equals(node.getNodeName()))
				{
					...
					if (cdata == null)
					{
						Log.w(LOG_TAG,"image <data> tag enclosed no " + "data. (empty data tag)");
					}
					else
					{
						byte[] imageData = Base64.decode(cdata.getNodeValue().trim(), Base64.DEFAULT);
	...
	private TileSet unmarshalTileset(Node t) throws Exception
	{
		...
		if (source != null)
		{
			...
			try
			{
				...
				{
					Log.w(LOG_TAG,"tileset files should end in .tsx! (" + source + ")");
				}
			...
			{
				Log.e(LOG_TAG,"Could not find external tileset file " + filename);
			}

			if (ext == null)
			{
				Log.e(LOG_TAG,"tileset " + source + " was not loaded correctly!");
				ext = new TileSet();
			}
		...
		else
		{
			...
			for (int i = 0; i < children.getLength(); i++)
			{
				...
				if (child.getNodeName().equalsIgnoreCase("image"))
				{
					if (hasTilesetImage)
					{
						Log.w(LOG_TAG,"Ignoring illegal image element after tileset image.");
						continue;
					}
					...
					if (imgSource != null && id == null)
					{
						...
						Log.i(LOG_TAG,"Importing " + sourcePath + "...");
	...
	private Tile unmarshalTile(TileSet set, Node t, String baseDir) throws Exception
	{
		...
		{
			Log.e(LOG_TAG,"failed creating tile: " + e.getLocalizedMessage());
			// e.printStackTrace();
			return tile;
		}
		...
		for (Node child = t.getFirstChild(); child != null; child = child.getNextSibling())
		{
			...
			if ("data".equalsIgnoreCase(nodeName))
			{
				...
				if (encoding != null && "base64".equalsIgnoreCase(encoding))
				{
					...
					if (cdata == null)
					{
						Log.w(LOG_TAG,"layer <data> tag enclosed no data. (empty data tag)");
					}
					else
					{
						byte[] dec = Base64.decode(cdata.getNodeValue().trim(),Base64.DEFAULT);
	...
	private void buildMap(Document doc) throws Exception
	{
		...
		{
			map = new AngleTileMap(mapWidth, mapHeight);
		}
		else
		{
	...
	private AngleTileMap unmarshal(InputStream in) throws Exception
	{
		DocumentBuilderFactory factory = DocumentBuilderFactory.newInstance();
		Document doc;
		try
	...
	public AngleTileMap readMap(String filename) throws Exception
	{
		xmlPath = filename.substring(0, filename.lastIndexOf(File.separatorChar) + 1);
		...
		AngleTileMap unmarshalledMap = unmarshal(is);
	...
	public AngleTileMap readMap(InputStream in) throws Exception
	{
		xmlPath = makeUrl(".");

		AngleTileMap unmarshalledMap = unmarshal(in);
